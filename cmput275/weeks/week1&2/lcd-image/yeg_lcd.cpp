...
int main() {
  ...
  // one screen down from above
  lcd_image_draw(&yegImage, &tft,
     // upper left corner in the image to draw
     yegMiddleX, yegMiddleY + DISPLAY_HEIGHT,
     // upper left corner of the screen to draw it in
     0, 0,
     // width and height of the patch of the image to draw
     DISPLAY_WIDTH - 471, DISPLAY_HEIGHT-311);
 
  /*
    Tasks
    1) Modify the code so the rightmost 60 columns of pixels are black.
    2) After a few seconds (e.g. delay(3000) or so), display the section
       of Edmonton that is half a screen down
  */
  Serial.end();
  return 0;
}